...
const staticStream = fastifyPlugin(function register(fastify, options, done) {
	...
	fastify.decorateReply("streamFile", function(file, root) {
		...
		if (typeof root !== "string") {
			console.error("[fastify-streamfile.streamFile] Type error: expected parameter `root` to be of type 'string'.");
			this.code(500);
			return;
		}
		...
		const extension_idx = file.lastIndexOf(".", (lastSlash === -1) ? file.length : lastSlash);
		// if there's an extension
		if (extension_idx > -1) {
			// set the content type to the correct mime type
			this.type(mime_dict[file.substring(extension_idx)] || "text");
		} else {
			// if there's no extension, set the content type to text
			this.type("text");
		}
		// send the file in a stream
		this.send(createReadStream(join(root, file)));
	});

	// add reply.sendFile
	fastify.decorateReply("sendFile", function(file, optional1, optional2) {
		/**
		 * reply.sendFile(file, ?optional1, ?optional2);
		 * 
		 * Sends a static file to the client.
		 * 
		 * Parameters:
		 * - file: the file to send
		 * - ?optional1: either the directory of the file to send or a callback function that receives the content of the file and must return content to be sent to the client
		 * - ?optional2: a callback function that receives the content of the file and must return content to be sent to the client
		 */
		// type checking (for nice errors)
		if (typeof file !== "string") {
			console.error("[fastify-streamfile.sendFile] Type error: expected parameter `file` to be of type 'string'.");
			this.code(500);
			return;
		}
		let callback = null;
		if (typeof optional1 === "string") {
			file = join(optional1, file);
			if (typeof optional2 === "function") {
				callback = optional2;
			} else if (typeof optional2 !== "undefined") {
				console.error("[fastify-streamfile.sendFile] Type error: expected parameter `optional2` to be of type 'function'.");
				this.code(500);
				return;
			}
		} else if (typeof optional1 === "function") {
			callback = optional1;
		} else if (typeof optional1 !== "undefined") {
			console.error("[fastify-streamfile.sendFile] Type error: expected parameter `optional1` to be of type 'string' or 'function'.");
			this.code(500);
			return;
		}
		...
		// read the file send contents once read
		readFile(file, function(err, data) {
			if (err) {
				console.error("[fastify-streamfile.sendFile] Failed to read file '" + file + "': " + err);
				this.code(500);
			} else {
	...
	// add fastify.static
	fastify.decorate("static", function(folder, root, route, depth) {
		/**
		 * fastify.static(folder, ?root, ?route);
		 * 
		 * Statically hosts all contents in the input folder at the input route.
		 * 
		 * Parameters:
		 * - folder: the folder containing the files to host
		 * - ?root: the parent directory of the file (defaults to "/")
		 * - ?route: the route to host the files at (defaults to "/static")
		 * - ?depth: the maximum depth to recurse into the folder (defaults to 10)
		 */
		// type checking (for nice errors)
		// max-recursion depth
		if (typeof depth === "undefined") {
			...
			console.error("[fastify-streamfile.streamFile] Type error: expected parameter `depth` to be of type 'number'.");
			this.code(500);
			return;
		} else if (depth < 0) {
			return;
		...
		if (typeof folder !== "string") {
			console.error("[fastify-streamfile.streamFile] Type error: expected parameter `folder` to be of type 'string'.");
			this.code(500);
			return;
		}
		if (typeof root !== "string") {
		...
		if (typeof route !== "string") {
			console.error("[fastify-streamfile.streamFile] Type error: expected parameter `route` to be of type 'string'.");
			this.code(500);
			return;
		}
		// read the folder
		try {
			...
			for (let i = 0; i < files.length; i++) {
				...
				const filePath = join(root, folder, files[i]);
				// get the route to the file
				const fileRoute = join(route, files[i]);
				...
				// if the file is a directory, recurse into it, otherwise host the file via a get route
				if (lstatSync(filePath).isDirectory()) {
					this.static(files[i], join(root, folder), fileRoute, depth - 1);
				} else {
					this.get(fileRoute, function(request, reply) {
						reply.streamFile(filePath);
					});
				}
			}
		} catch (err) {
			console.error("[fastify-streamfile.streamFile] Failed to read folder '" + folder + "': " + err);
			this.code(500);
			return;
		}
	});
